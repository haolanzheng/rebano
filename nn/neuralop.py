...
class SpectralConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.fft.irfft2(out_ft, s=(x.size(-2), x.size(-1)), dim=[2,3])
        return x

class FNO2d(nn.Module):
    def __init__(self, modes1, modes2,  width, in_dim=3):
...
class FNO1d(nn.Module):
    def __init__(self, modes, width, in_dim=2):
        ...
        self.fc0 = nn.Linear(in_dim, self.width) # input channel is 2: (a(x), x)
