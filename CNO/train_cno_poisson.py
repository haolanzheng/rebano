...
from CNO1d_vanilla_torch_version.CNO1d import CNO1d
...
def main(args):
    ...
    print("\nNx: ", Nx)
    x_data = torch.from_numpy(np.load(f"../Poisson/data/poisson1d_input_f_K_10_s{Nx}.npy")).type(torch.float32)
    y_data = torch.from_numpy(np.load(f"../Poisson/data/poisson1d_output_u_K_10_s{Nx}.npy")).type(torch.float32)
    ...
    if args.train:
        ...
        cno_pred_sol_train = torch.zeros(s, input_function_train.shape[0], dtype=torch.float).to(device)
        ...
        with torch.no_grad():
            ...
            for step, (input_batch, output_batch) in enumerate(testing_set):
                ...
                cno_pred_sol_train[:, step] = output_pred_batch.reshape(-1)
                cno_rel_err_train[step] = loss_f.item()
            test_relative_l2 /= len(testing_set)
        cno_pred_sol_train = cno_pred_sol_train.cpu().numpy()
        # np.save(f'./data/CNO_Poisson_solutions_train_K_10_n_{n_train}.npy', cno_pred_sol_train)
        # np.save(f'./data/CNO_Poisson_rel_err_train_K_10_n_{n_train}.npy', cno_rel_err_train)
        print(f"Avg Inference Time : {total_time/n_train} seconds")
        print("Mean Relative L2 Test Error:", np.mean(cno_rel_err))
        print("Max Relative L2 Test Error:", np.max(cno_rel_err))
    
    
    
    if args.test:
        testing_set = DataLoader(TensorDataset(input_function_test, output_function_test), batch_size=1, shuffle=False)
        cno_pred_sol = torch.zeros(s, input_function_test.shape[0], dtype=torch.float).to(device)
        cno_rel_err  = np.zeros(input_function_test.shape[0])
        if args.ckpt:
            ckpt_path = args.ckpt
        ...
        with torch.no_grad():
            ...
            for step, (input_batch, output_batch) in enumerate(testing_set):
                ...
                total_time += t1 - t0
                test_relative_l2 += loss_f.item()
                cno_pred_sol[:, step] = output_pred_batch.reshape(-1)
                cno_rel_err[step] = loss_f.item()
            test_relative_l2 /= len(testing_set)
        cno_pred_sol = cno_pred_sol.cpu().numpy()
        np.save(data_path + f'CNO_Poisson_solutions_test_K_10_n_{n_test}_{Nx}.npy', cno_pred_sol)
        np.save(data_path + f'CNO_Poisson_rel_err_test_K_10_n_{n_test}_{Nx}.npy', cno_rel_err)
