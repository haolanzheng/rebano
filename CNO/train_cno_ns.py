...
def main(args):
    ...
    Nx = 100
    x_data = torch.from_numpy(np.load(f"../NS/data/ns_input_curl_f_K_8_s{Nx}.npy"))
    y_data = torch.from_numpy(np.load(f"../NS/data/ns_output_omega_K_8_s{Nx}.npy"))
    ...
    if args.train:
        ...
        save_ckpt(f'ckpts/ns-cno-{epochs}.pt', cno, optimizer, scheduler)
        ...
        np.save(data_path + f'CNO_NS_solutions_train_K_8_n_{n_train}_{s}.npy', cno_pred_sol_train)
        np.save(data_path + f'CNO_NS_rel_err_train_K_8_n_{n_train}_{s}.npy', cno_rel_err_train)
    ...
    if args.test:
        ...
        np.save(data_path + f'CNO_NS_solutions_test_K_8_n_{n_test}_{s}.npy', cno_pred_sol_test)
        np.save(data_path + f'CNO_NS_rel_err_test_K_8_n_{n_test}_{s}.npy', cno_rel_err_test)
        print(f"Avg Inference Time : {total_time/n_test} seconds")
        print("Mean Relative L2 Train Error:", np.mean(cno_rel_err_test))
        print("Max Relative L2 Train Error:", np.max(cno_rel_err_test))
