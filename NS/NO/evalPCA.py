...
train_outputs = np.reshape(train_outputs, (Nx, Nx, n_train))
test_outputs  = np.reshape(test_outputs, (Nx, Nx, n_test))
...
with torch.no_grad():
    ...
    for i in range(n_train):
        t0 = perf_counter()
        y_pred = model(x_train[i:i+1]).detach()
        y_pred_train[:, :, i] = torch.matmul(Ug, y_pred.T).reshape(Nx, Nx)
        t1 = perf_counter()
        total_time += (t1 - t0)
...
with torch.no_grad():
    ...
    for i in range(n_test):
        t0 = perf_counter()
        y_pred = model(x_test[i:i+1]).detach()
        y_pred_test[:, :, i] = torch.matmul(Ug, y_pred.T).reshape(Nx, Nx)
        t1 = perf_counter()
        total_time += (t1 - t0)
